...
function seedRun(db, conversationId) {
  const windowConfigId = Number(
    db
      .prepare(
        `
        INSERT INTO window_configs (name, message_count, stride, min_tail_messages)
        VALUES ('smoke-10-by-10', 10, 10, 10)
      `,
      )
      .run().lastInsertRowid,
  )
  const scorerConfigId = Number(
    db
      .prepare(
        `
        INSERT INTO scorer_configs (key, label, config_json)
        VALUES ('smoke-baseline-v1', 'Smoke baseline', '{}')
      `,
      )
      .run().lastInsertRowid,
  )
  ...
  scores.forEach((score, index) => {
    ...
    insertWindow.run(
      ...
      startOrdinal,
      endOrdinal,
      1_700_000_000_000 + startOrdinal,
      1_700_000_000_000 + endOrdinal,
