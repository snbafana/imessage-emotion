...
export const SCORE_KEYS = ['warmth', 'joy', 'stress', 'friction', 'sadness'];
export function hasConversationApi(api) {
...
export function normalizeConversations(input) {
    return unwrapList(input, ['conversations', 'items', 'rows']).map((item) => {
        ...
        const title = getString(row, ['title', 'displayName', 'display_name', 'name', 'chat_identifier']) ??
            `Conversation ${rawId}`;
        const participantSummary = getString(row, ['participantSummary', 'participant_summary', 'participants']) ?? title;
...
export function normalizeWindows(input) {
    return unwrapList(input, ['windows', 'items', 'rows']).map((item, index) => {
        ...
        const focalStartOrdinal = getNumber(row, ['focalStartOrdinal', 'focal_start_ordinal']) ?? startOrdinal;
        ...
        const contextMessageCount = getNumber(row, ['contextMessageCount', 'context_message_count']) ??
            ordinalCount(contextStartOrdinal, contextEndOrdinal);
        const focalMessageCount = getNumber(row, ['focalMessageCount', 'focal_message_count']) ??
...
export function normalizeMessages(input) {
    return unwrapList(input, ['messages', 'items', 'rows']).map((item) => {
        ...
        const sender = getString(row, ['senderName', 'sender_name', 'displayName', 'display_name']) ??
            (isFromMe ? 'You' : 'Them');
        return {
            id: String(rawId),
            rawId,
...
export async function getWindowMessages(api, windowId, slice) {
...
export function timelineBlocks(windows) {
...
export function runStateLabel(run, windows) {
    if (!run)
        return 'No baseline run yet';
    if (run.state === 'failed')
        return 'Run failed';
...
export function gradientFor(composition) {
...
export function formatDateRange(first, last) {
    ...
    if (first == null)
        return `through ${formatMonthYear(last)}`;
    if (last == null)
        return `from ${formatMonthYear(first)}`;
    return `${formatMonthYear(first)} - ${formatMonthYear(last)}`;
}
export function renderDashboardSmokeHtml(input) {
...
function compositionForScores(scores, dominant) {
...
function extractScores(result) {
    ...
    return SCORE_KEYS.reduce((scores, key) => {
        ...
        if (value != null)
            scores[key] = clamp01(value);
        return scores;
    }, {});
}
function getDominantScore(result, scores) {
    const dominant = getString(result, ['dominant']);
    if (isScoreKey(dominant))
        return dominant;
    return SCORE_KEYS.reduce((best, key) => {
        if (scores[key] == null)
            return best;
        if (best == null || (scores[key] ?? 0) > (scores[best] ?? 0))
            return key;
        return best;
    }, null);
}
function isScoreKey(value) {
    return SCORE_KEYS.includes(value);
}
function unwrapList(input, keys) {
...
function getOptionalId(row, keys) {
    const value = getValue(row, keys);
    if (typeof value === 'number' || typeof value === 'string')
        return value;
    return null;
}
function getTime(row, keys) {
    const value = getValue(row, keys);
    if (typeof value === 'number' && Number.isFinite(value))
        return value;
    if (typeof value === 'string' && value.trim().length > 0) {
...
function ordinalCount(start, end) {
